...
-- safeClick: filtert auto-shot-clicks (die triggern sonst zufällig menu-buttons)
local function safeClick(btn, cb)
    btn.MouseButton1Click:Connect(function(...)
        -- Wenn innerhalb 100ms nach auto-shot fire → ignorieren
        local t = _G.ENI_AUTOSHOT_FIRING
        if t and tick() - t < 0.3 then return end
        cb(...)
    end)
end
...
local C = {
    -- Aimbot
    AimEnabled     = true,
    MobileAimButton = true,  -- Mobile: zeigt AIM knopf on-screen
    ...
    ItemESP_Food    = true,
    ItemESP_Chest    = true,
    ItemESP_Bookshelf= true,
    ItemESP_Shelf    = true,
    ItemESP_MaxDistance = 500,
    VehicleESP     = false,
    DeadBodyESP    = false,
    ...
    AutoShot       = false,
    AutoShotDelay  = 250,  -- default höher auf mobile (vermeidet input-flood)
    HitSound       = false,
    HitSoundType   = "Bubble",
    HitSoundVolume = 1.0,
    ...
    Freecam        = false,
    MobileFreecamButton = true,  -- Mobile: zeigt CAM knopf on-screen
    FreecamSpeed   = 50,
    FreecamKey     = Enum.KeyCode.Unknown,  -- kein default keybind
    FlyKey         = Enum.KeyCode.Unknown,   -- kein default keybind
    AimToggleKey   = Enum.KeyCode.Unknown,   -- kein default keybind (togglet aim button)
    -- Settings
...
local IS_MOBILE = true --[[MOBILE FORCED]]
local TAB_W, TAB_H = 80, 26
if IS_MOBILE then
    TAB_W, TAB_H = 78, 40  -- schmaler damit alle 8 tabs reinpassen (78*8=624px + padding)
    ...
    -- Menu insgesamt kompakter für kleinen bildschirm — via UIScale
    -- FIX: menuScale wird erst später deklariert; FindFirstChildOfClass dynamisch statt upvalue-lookup
    task.spawn(function()
        for _ = 1, 20 do  -- retry loop damit's wirklich greift
            task.wait(0.2)
            local scale = Main:FindFirstChildOfClass("UIScale")
            if scale then
                scale.Scale = 0.55  -- 55% = handy-tauglich
                break
            end
        end
    end)
    ...
    task.spawn(function()
        task.wait(1.5)  -- warte bis launch animation fertig
        local viewport = Camera.ViewportSize
        if viewport.X < 900 then
            -- Kleines display → oben mittig
...
local function AddTab(name, icon)
    ...
    Pages[name] = page
    safeClick(btn, function() selectTab(name) end)
...
local function Toggle(parent, label, key)
    ...
    -- Down+Up cycle für mobile-freundliche tap detection
...
Toggle(pCombat, "Mobile Aim Button (on-screen toggle)", "MobileAimButton")
...
Toggle(pCombat, "Auto Shot (may not work on some games)", "AutoShot")
task.spawn(function()
...
Toggle(pCombat, "Rapid Fire (may not work on most games)", "RapidFire")
Slider(pCombat, "Fire Rate (ms delay)", "FireRate", 15, 200, 5)
Toggle(pCombat, "Hitbox Expander (bigger enemy hitboxes)", "HitboxExpander")
Slider(pCombat, "Hitbox Size (studs)", "HitboxSize", 3, 25, 1)
...
Section(pMov, "Speed")
Toggle(pMov, "Speed Hack", "SpeedEnabled")
Slider(pMov, "Walk Speed", "SpeedValue", 16, 100, 1)
Section(pMov, "Jump")
Toggle(pMov, "Jump Boost", "JumpBoost")
Slider(pMov, "Jump Power", "JumpValue", 50, 300, 5)
Toggle(pMov, "Infinite Jump", "InfJump")
Section(pMov, "Fly")
Toggle(pMov, "Fly (WASD + Space/Ctrl)", "FlyEnabled")
Slider(pMov, "Fly Speed", "FlySpeed", 20, 250, 5)
...
Toggle(pMisc, "Mobile Freecam Button (on-screen)", "MobileFreecamButton")
...
do
    ...
    local function addEntry(p)
        ...
        safeClick(row, function() teleportTo(p) end)
...
KeybindPicker(pSet, "Toggle Aim (mobile button)", "AimToggleKey")
...
-- Eigene ScreenGui mit hoher DisplayOrder → garantiert immer sichtbar
-- Grösser + auffälliger für Handy-User
do
    local ToggleGui = New("ScreenGui", {
        Name = "ENI_TOGGLE_GUI", Parent = GuiParent,
        ResetOnSpawn = false, IgnoreGuiInset = true,
        DisplayOrder = 1500,  -- über dem Menu (1000), unter dem Loader (2000)
        ZIndexBehavior = Enum.ZIndexBehavior.Sibling,
    })
    local ToggleBtn = New("TextButton", {
        Parent = ToggleGui, Name = "ENI_TOGGLE",
        Size = UDim2.fromOffset(70, 70),  -- deutlich grösser
        Position = UDim2.new(0, 15, 0, 100),  -- links oben, unter Roblox top-bar
        BackgroundColor3 = Color3.fromRGB(0, 0, 0), BorderSizePixel = 0,
        Text = "ENI", Font = Enum.Font.GothamBlack, TextSize = 18,
    ...
    -- MouseButton1Down/Up + Click für sauberes tap-detection
    local pressed = false
    local moved   = false
    ...
    ToggleBtn.MouseButton1Up:Connect(function()
        if not pressed then return end  -- fake up ohne down → ignorieren
        pressed = false
        if moved then moved = false; return end
        -- Auto-shot check: wenn's ne auto-shot fire war → ignorieren
        local t = _G.ENI_AUTOSHOT_FIRING
        if t and tick() - t < 0.3 then return end
        -- Debounce: min 250ms zwischen toggles (blockt spurious rapid-fire clicks)
        if tick() - lastToggle < 0.25 then return end
        lastToggle = tick()
        Main.Visible = not Main.Visible
        Tween(ToggleBtn, 0.08, {Size = UDim2.fromOffset(62, 62)})
        task.delay(0.08, function()
            Tween(ToggleBtn, 0.12, {Size = UDim2.fromOffset(70, 70)})
        end)
    end)
end
...
if true --[[MOBILE FORCED]] then
    _G.ENI_AIM_MOBILE_ACTIVE = false

    local COLOR_OFF = Color3.fromRGB(200, 40, 55)   -- rot wenn aim OFF
    local COLOR_ON  = Color3.fromRGB(50, 200, 90)   -- grün wenn aim ON
    local AimBtn = New("TextButton", {
        Parent = Gui, Name = "ENI_AIM_TOGGLE",
        Size = UDim2.fromOffset(60, 60),
        Position = UDim2.new(1, -80, 0.5, -30),  -- rechts, mittig vertikal
        BackgroundColor3 = COLOR_OFF, BorderSizePixel = 0,
        Text = "AIM", Font = Enum.Font.GothamBlack, TextSize = 15,
        TextColor3 = Color3.fromRGB(255, 255, 255),
        AutoButtonColor = false, ZIndex = 100,
        Active = true, Draggable = false, Visible = false,
    })
    ...
    -- Watchdog: zeigt Button nur wenn Aimbot on UND MobileAimButton toggle on
    task.spawn(function()
        while AimBtn.Parent do
            task.wait(0.2)
            local shouldShow = C.AimEnabled and C.MobileAimButton
            if shouldShow and not AimBtn.Visible then
                AimBtn.Visible = true
            elseif not shouldShow and AimBtn.Visible then
                AimBtn.Visible = false
                _G.ENI_AIM_MOBILE_ACTIVE = false  -- reset wenn button verschwindet
            end
        end
    end)

    -- MouseButton1Down/Up + Click für sauberes tap-detection
    -- (InputBegan/Ended feuerten auch für hover-events auf PC → auto-toggle bug)
    local pressed = false
    local moved   = false
    local dragStart, startPos
    local DRAG_TH  = 6
    ...
    local lastAimToggle = 0
    AimBtn.MouseButton1Up:Connect(function()
        if not pressed then return end
        pressed = false
        if moved then moved = false; return end
        -- Debounce: min 250ms zwischen toggles
        if tick() - lastAimToggle < 0.25 then return end
        lastAimToggle = tick()
        -- Tap → toggle aim
        _G.ENI_AIM_MOBILE_ACTIVE = not _G.ENI_AIM_MOBILE_ACTIVE
        if _G.ENI_AIM_MOBILE_ACTIVE then
            AimBtn.BackgroundColor3 = COLOR_ON
            aimStroke.Color         = Color3.fromRGB(120, 255, 150)
            aimStroke.Transparency  = 0
            AimBtn.TextColor3       = Color3.fromRGB(255, 255, 255)
        else
            AimBtn.BackgroundColor3 = COLOR_OFF
            aimStroke.Color         = Color3.fromRGB(255, 100, 120)
...
-- ═════════ MOBILE FREECAM TOGGLE BUTTON ═════════
-- Erscheint als weißer runder Button rechts unten. Tap → Freecam an/aus.
-- Draggable damit user ihn hinschieben kann wo er möchte.
if true --[[MOBILE FORCED]] then
    local COLOR_CAM_OFF = Color3.fromRGB(40, 40, 55)
    local COLOR_CAM_ON  = Color3.fromRGB(255, 200, 60)  -- orange wenn aktiv

    local CamGui = New("ScreenGui", {
        Name = "ENI_CAM_TOGGLE_GUI", Parent = GuiParent,
        ResetOnSpawn = false, IgnoreGuiInset = true,
        DisplayOrder = 1500,
        ZIndexBehavior = Enum.ZIndexBehavior.Sibling,
    })
    local CamBtn = New("TextButton", {
        Parent = CamGui, Name = "ENI_CAM_TOGGLE",
        Size = UDim2.fromOffset(60, 60),
        Position = UDim2.new(1, -80, 0.5, 50),  -- rechts, unter aim button
        BackgroundColor3 = COLOR_CAM_OFF, BorderSizePixel = 0,
        Text = "CAM", Font = Enum.Font.GothamBlack, TextSize = 15,
        TextColor3 = Color3.fromRGB(255, 255, 255),
        AutoButtonColor = false, ZIndex = 100,
        Active = true, Draggable = false,
    })
    New("UICorner", {Parent = CamBtn, CornerRadius = UDim.new(1, 0)})
    local camStroke = New("UIStroke", {Parent = CamBtn,
        Color = Color3.fromRGB(255, 255, 255), Thickness = 2, Transparency = 0.4})

    -- Watchdog: zeigt/versteckt Button je nach C.MobileFreecamButton toggle
    task.spawn(function()
        while CamBtn.Parent do
            task.wait(0.2)
            local shouldShow = C.MobileFreecamButton ~= false
            if shouldShow and not CamBtn.Visible then
                CamBtn.Visible = true
            elseif not shouldShow and CamBtn.Visible then
                CamBtn.Visible = false
            end
        end
    end)

    -- Drag/tap logic (gleiche pattern wie AIM button)
    local pressed = false
    local moved   = false
    local dragStart, startPos
    local DRAG_TH = 6
    ...
    local pressTime = 0
    CamBtn.MouseButton1Down:Connect(function()
        pressed   = true
        pressTime = tick()
        moved     = false
        dragStart = UIS:GetMouseLocation()
        startPos  = CamBtn.Position
    end)
    ...
    local lastCamToggle = 0
    CamBtn.MouseButton1Up:Connect(function()
        if not pressed then return end
        pressed = false
        if moved then moved = false; return end
        -- Debounce: min 250ms zwischen toggles
        if tick() - lastCamToggle < 0.25 then return end
        lastCamToggle = tick()
        C.Freecam = not C.Freecam
        if C.Freecam then
            CamBtn.BackgroundColor3 = COLOR_CAM_ON
            camStroke.Color         = Color3.fromRGB(255, 240, 140)
            camStroke.Transparency  = 0
        else
            CamBtn.BackgroundColor3 = COLOR_CAM_OFF
            camStroke.Color         = Color3.fromRGB(255, 255, 255)
            camStroke.Transparency  = 0.4
        end
        Tween(CamBtn, 0.08, {Size = UDim2.fromOffset(54, 54)})
        task.delay(0.08, function()
            Tween(CamBtn, 0.12, {Size = UDim2.fromOffset(60, 60)})
        end)
    end)
end
...
safeClick(CloseBtn, function() Preview:Destroy() end)
...
RunService.RenderStepped:Connect(function()
    ...
    -- MOBILE-fix: NUR mini-nudges pro frame in richtung target (kein hartes camera-lock)
    -- Fightet nicht mit dem mobile camera-controller → kein freeze
    local currentLook = Camera.CFrame.LookVector
    local desiredLook = (part.Position - Camera.CFrame.Position).Unit
    -- Interpoliere den look-vector sanft (nicht das ganze CFrame)
    local newLook = currentLook:Lerp(desiredLook, math.clamp(1 - math.max(C.AimSmoothness, 0.35), 0.03, 0.4))
...
task.spawn(function()
    while true do
        ...
        -- MOBILE: nur tool:Activate + RemoteEvent (keine mouse events → keine menu-clicks)
...
-- ═════════════════════════════════════════════════
--   MOVEMENT: Da Hood-proof (CFrame velocity)
--   Da Hood resettet WalkSpeed constantly, deshalb
--   verschieben wir den Character manuell per Frame.
-- ═════════════════════════════════════════════════
RunService.Heartbeat:Connect(function(dt)
    ...
    -- Only boost when player is actually moving (WASD pressed)
    if hum.MoveDirection.Magnitude > 0 then
        local extra = C.SpeedValue - 16
        if extra > 0 then
            root.CFrame = root.CFrame + hum.MoveDirection * extra * dt
        end
    end
end)
...
task.spawn(function()
    while true do
        ...
        if C.RapidFire and mouseButtonHeld then
            ...
            if tool then
                -- MOBILE: nur tool:Activate (keine mouse events → keine menu-clicks)
                pcall(function() tool:Activate() end)
            end
        end
    end
...
UIS.InputBegan:Connect(function(input, _gp)
    ...
    elseif input.KeyCode == C.AimToggleKey and C.AimToggleKey ~= Enum.KeyCode.Unknown then
        _G.ENI_AIM_MOBILE_ACTIVE = not _G.ENI_AIM_MOBILE_ACTIVE
    end
end)
...
print("[KUNI PRIVATE - MOBILE] loaded")
