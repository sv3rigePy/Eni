...
-- ═════ ENI REAL TEST — executor kompatibilitäts-check ═════
warn("[ENI REAL TEST] SCRIPT STARTED")

-- Check welche APIs verfügbar sind + report
local apis = {
...
for name, fn in pairs(apis) do
    if fn then table.insert(have, name) else table.insert(missing, name) end
end
warn("[ENI REAL TEST] APIs verfügbar: " .. table.concat(have, ", "))
warn("[ENI REAL TEST] APIs fehlen:    " .. table.concat(missing, ", "))

pcall(function()
    game.StarterGui:SetCore("SendNotification", {
        Title = "ENI Real Test",
        Text  = #missing .. " APIs fehlen — check F9 für details",
        Duration = 4,
    })
end)
...
-- Real executor kompatibilität: wenn kein readfile/writefile → login skip
local loginOK, loginErr = true, nil
if readfile and writefile and isfile then
    loginOK, loginErr = pcall(showLoginGui)
else
    warn("[ENI REAL TEST] readfile/writefile fehlt → login übersprungen (executor: probably Real)")
end
...
print("[ENI HUB - REAL TEST] loaded")
